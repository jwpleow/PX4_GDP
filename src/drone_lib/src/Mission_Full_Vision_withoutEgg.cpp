...
int main(int argc, char **argv)
{
    ...
    // MISSION STARTS HERE:
    // Request takeoff at 1m altitude. At 10Hz = 5 seconds
    float altitude = 1.0;
    int time_takeoff = 50;
    ...
    ///<---------------------------------- STEP 3 ---- AMBULANCE TRACKING --- JAKE PN---------------------------->
    ...
    float descentVelocity = -0.2f;
    float descentDistance = 0.15f;
    float LandAlt = 0.3f;
    ...
    while(camdistance > descentDistance || LandAlt < altitude)
    {
        ...
        {
            ROS_INFO("tag seen but far - using vishnu data to centre over target");
        ...
        else if (camdistance < 0.05)
        {
            ROS_INFO("Very near target - stabilising");
            ////< Stabilise
            ...
            if (camdistance < 0.05) ///< if cam distance still near, land
            {
